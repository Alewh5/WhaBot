...
def recibir_mensajes():
    try:
        ...
        if 'es todo' in text:
            ...
            response = services.generar_respuesta_chatgpt(text,number,False)
            services.guardar_conversacion(messageId,number,name,text,timestamp, response)
            data = services.text_Message(number, response)
