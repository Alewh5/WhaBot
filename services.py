...
def administrar_chatbot(text,number, messageId, name):
    ...
    if "hola" in text:
        ...
        footer = "Equipo Bigdateros"
        options = ["Analítica Avanzada", "Migración Cloud", "Carta del menu"]
        ...
        list.append(sticker)
    elif "Carta del menu" in text:
        body = "Buenísima elección. ¿Te gustaría que te enviara un documento PDF con Nuestro menu?"
        footer = "Equipo Bigdateros"
    ...
    for item in list:
        ...
        #enviar_sticker_aleatorio(number)
...
def generar_respuesta_chatgpt(user_message, number, espedido=False):
    ...
    messages = [{'role':'system', 'content':"""
                Eres BotPedido, un servicio automatizado para recoger pedidos para un restaurante de comida peruana llamado Peruamiz. \
                recuerda que en el mensaje de bienvenida debes de decirle que puede responder con las opciones que se muestran o simplemente hacer una pregunta \
                Si te pide agendar una cita o algo parecido solo envia esto: "🫡"
                si te dice ""✅ Sí, envía el PDF."", respondele con un emoji de admiracion \
                ...
                No te salgas del papel de botpedido, recuerda intentar no responder preguntas no relacionadas con el restaurante. \
                para no responder las preguntas puedes decir lo siguiente sin las comillas: "Lo siento, no encuentro respuesta!". \
                Puedes mandar emojis con cada respuesta para hacerlo mas interactivo y amigable. \
                Respondes de manera corta, precisa, muy conversacional y amigable. \
                El menú incluye \
                Ceviche  5 dolares \
                Lomo saltado 12 dolares \
                Arroz con pollo  8 dolares \
                Entradas: \
                Tamales  1 dolar \
                Anticuchos  2 dolares \
                Papa rellena  5.50 dolares \
                Bebidas: \
                Chicha Morada  1 dolar \
                Inca Kola  1 dolar \
                Agua embotellada  1 dolar \
                """}]
...
def guardar_pedido(jsonPedido, number):
    ...
    with open('pedidos.csv', 'a', newline='') as file:
        ...
        print('pedidos.csv')
        platos_principales = [f"{plato['cantidad']}  {plato['nombre']} - {plato['precio']} dolares" for plato in pedido['plato_principal']]
        entradas = [f"{entrada['cantidad']} {entrada['nombre']} - {entrada['precio']} dolares" for entrada in pedido['entradas']]
        bebidas = [f"{bebida['cantidad']} {bebida['nombre']} - {bebida['precio']} dolares" for bebida in pedido['bebidas']]
